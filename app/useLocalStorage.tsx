...
export function useWindowEvent<K extends string = keyof WindowEventMap> (
  ...
  useEffect(() => {
    if (!window) return
    // @ts-expect-error enable custom event
...
export function createStorage<T> (type: StorageType, hookName: string) {
  const eventName = type === 'localStorage' ? 'zeroroku-local-storage' : 'zeroroku-session-storage'
  ...
  return function useStorage ({
    ...
    }, [getInitialValueInEffect, key, readStorageValue])
